# Hi 👋, I'm Lavanya V

🚀 **Full Stack Developer | AI Enthusiast**

I am a Computer Science (AI & ML) student passionate about building real-world web applications and exploring AI systems.

---

## 👨‍💻 About Me

* 🎓 B.E CCE student at Sri Eshwar College of Engineering
* 💻 MERN Stack Intern at Better Tomorrow
* 🌐 Modern Full Stack Intern at Algo Tutor
* 🧠 Interested in Web Development, AI, and IoT Systems
* ⚡ Strong in DSA, DBMS, and OOP concepts

---

## 🛠️ Technical Skills

### 💻 Programming

* C++
* JavaScript

### 🌐 Web Development

* HTML, CSS, JavaScript
* React.js
* Node.js
* Express.js
* MongoDB

### 🛠️ Tools

* Git & GitHub
* VS Code
* Postman
* Power BI
* Excel
* Canva
* MongoDB Compass

---

## 💼 Internship Experience

### MERN Stack Intern — Better Tomorrow

* Developed full-stack web applications
* Built responsive UI and backend APIs
* Improved performance and fixed bugs

### Modern Full Stack Intern — Algo Tutor

* Worked with Next.js and MongoDB
* Built scalable applications
* Developed responsive UI

---

## 🚀 Projects

### 🍽️ Online Recipe Management

* Recipe creation & dashboard
* Search + authentication system

### 🐔 IoT Poultry Farm Automation

* Automated feeding & temperature control
* Remote monitoring system

### 🌿 EcoFarm Explorer

* Booking platform with authentication
* Search & filter system

---

## 🏆 Achievements

* 🥇 1st Place — Project Expo
* 🥇 First Prize — Paper Presentation

---

## 📜 Certifications

* Responsible & Safe AI — NPTEL
* C++ — Saylor Academy
* Data Structures — Udemy
* Python — Infosys
* Java — Udemy

---

## 💻 Coding Profiles

* LeetCode — 250+ problems (Rating: 1485)
* CodeChef — 300+ problems (Rating: 946)
* SkillRack — 400+ problems

---

## 📫 Connect With Me

* GitHub: https://github.com/lavanya-vasanth
* Email: [lavanya.v2023cce@sece.ac.in](mailto:lavanya.v2023cce@sece.ac.in)
